...
struct device_bundle_t {
    ...
    result_t average(const std::function<result_t(const device_t &)> &block) const {
        std::vector<result_t> result(count);
        std::transform(devices, devices + count, result.begin(), block);
        return std::accumulate(result.begin(), result.end(), result_t{}) / count;
    }
};
...
enum class odometry_type : uint8_t {
    state, delta
};
...
public:
    ...
    double left_position() {
        return to_radian(AVERAGE(double, left, { return it.get_position(); }));
    }

    double right_position() {
        return to_radian(AVERAGE(double, right, { return it.get_position(); }));
    }
